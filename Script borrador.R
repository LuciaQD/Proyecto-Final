...
#Crear variable a?o
base2013<-base2013%>%mutate(a?o=2013)
base2014<-base2014%>%mutate(a?o=2014)
base2015<-base2015%>%mutate(a?o=2015)
base2016<-base2016%>%mutate(a?o=2016)
base2017<-base2017%>%mutate(a?o=2017)
...
ggplot(datos,aes(x = a?o)) + geom_bar()
...
ggplot(datos,aes(x = a?o,fill=Gravedad)) + geom_bar(position = "fill")#Parece que hay una tendencia en los no lesionados
ggplot(datos,aes(fill = datos$`Tipo de Siniestro`,x=a?o)) + geom_bar(position = "fill")
